...
export const driversService = {
  ...
  async getPendingDocuments(): Promise<DriverDocument[]> {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        driver:profiles!driver_documents_driver_id_fkey(id, nombre, apellido, email, telefono_numero, telefono_codigo_pais, driver_status, driver_type, avatar)
      `)
      .eq('status', 'pending')
      .order('created_at', { ascending: false });
  ...
  // Aprobar documento.
  // Va por el BACKEND (no Supabase directo) para que el endpoint admin
  // dispare checkAndActivateDriver: al aprobar el último doc requerido,
  // el cadete pasa automáticamente a driver_status='active'.
  // (reviewerId queda implícito: el backend usa el admin autenticado)
  async approveDocument(id: string, _reviewerId?: string): Promise<DriverDocument> {
    const { data } = await api.put(`/drivers/admin/documents/${id}`, {
      status: 'approved',
    });
    return (data?.document ?? data) as DriverDocument;
  },

  // Rechazar documento (también vía backend admin)
  ...
  // Aprobar/activar un CADETE manualmente (estado del perfil), vía backend admin.
  // status: 'active' | 'suspended' | 'pending_documents' | 'pending_review'
  async setDriverStatus(driverId: string, status: string, reason?: string) {
    const { data } = await api.put(`/drivers/admin/${driverId}/status`, {
      status,
      reason,
    });
    return data;
