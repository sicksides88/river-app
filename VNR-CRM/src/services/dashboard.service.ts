...
export const dashboardService = {
  ...
  async getStats(): Promise<DashboardStats> {
    const [
      ...
      supabase.from('profiles').select('role, driver_status, is_verified, is_driver'),
    ...
    return {
      ...
      totalDrivers: profiles.filter(p => p.is_driver || p.role === 'driver').length,
      // driver_status válidos: pending_documents | pending_review | active | suspended
      pendingDrivers: profiles.filter(p => p.driver_status === 'pending_documents' || p.driver_status === 'pending_review').length,
      activeDrivers: profiles.filter(p => p.driver_status === 'active').length,
