...
const TripCompletedScreen = ({ navigation, route }) => {
  ...
  const handleGoHome = () => {
    // Antes de volver al inicio, ofrecer calificar al pasajero/cliente.
    // Si no tenemos a quién calificar, vamos directo a la home.
    const ratedId =
      trip?.user?.id || trip?.userId || trip?.user_id || trip?.passenger?.id;
    if (ratedId) {
      navigation.replace('RateUser', {
        isDelivery,
        rideId: isDelivery ? undefined : trip?.id,
        deliveryId: isDelivery ? trip?.deliveryId || trip?.id : undefined,
        ratedId,
        user: trip?.user || trip?.passenger,
      });
    } else {
      goHome();
    }
      ...
      <View style={styles.bottomContainer}>
        {isPendingPayment ? (
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={styles.homeButtonText}>Calificar y finalizar</Text>
          </TouchableOpacity>
        )}
      </View>
    </SafeAreaView>
