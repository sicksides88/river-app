...
// Qué servicios puede ofrecer cada tipo de vehículo (capacidad real):
// auto = pasajeros + envíos; moto = solo envíos; camioneta/furgón = fletes + envíos.
const VEHICLE_SERVICE_MAP = {
  moto: ['cadete'],
  bici: ['cadete'],
  bicicleta: ['cadete'],
  auto: ['vuelta_segura', 'chofer', 'cadete'],
  sedan: ['vuelta_segura', 'chofer', 'cadete'],
  camioneta: ['fletes', 'cadete'],
  furgon: ['fletes', 'cadete'],
  pickup: ['fletes', 'cadete'],
};
...
const DriverHomeScreen = ({ navigation }) => {
  ...
  // Servicios habilitados según la CAPACIDAD del vehículo registrado:
  // un auto no puede hacer fletes, una moto solo envíos, etc.
  const enabledServiceIds =
    Array.isArray(user?.selected_services) && user.selected_services.length > 0
      ? user.selected_services
      : (user?.driver_type ? [user.driver_type] : null);
  const vehicleAllowedIds = currentVehicle?.vehicle_type
    ? (VEHICLE_SERVICE_MAP[String(currentVehicle.vehicle_type).toLowerCase()] || null)
    : null;
  // Prioridad: capacidad del vehículo → lo que eligió el chofer → todos.
  const allowedIds = vehicleAllowedIds || enabledServiceIds;
  const filteredServices = allowedIds
    ? SERVICE_OPTIONS.filter((s) => allowedIds.includes(s.id))
    : [];
  const baseServices = filteredServices.length > 0 ? filteredServices : SERVICE_OPTIONS;
  const availableServices = enabledServices
    ? baseServices.filter((s) => enabledServices.includes(toCanonicalService(s.id)))
    : baseServices;
  ...
  const { isConnected: socketConnected, goOnline, goOffline } = useDriverSocket({
    onNewRideRequest: (data) => {
      if (isActive) {
        ...
        // App en 2do plano: avisar con notificación local (la pantalla queda lista).
        if (AppState.currentState !== 'active') {
          driverPresence.notifyNewOrder({
            title: 'Nuevo viaje disponible',
            body: `${tripData.type}${data.estimatedPrice ? ` · $${data.estimatedPrice}` : ''}`,
            data: { type: 'driver_new_order', rideId: data.rideId },
          });
        }
      ...
      if (isActive) {
        ...
        if (AppState.currentState !== 'active') {
          driverPresence.notifyNewOrder({
            ...
            body: `${tripData.type}${data.estimatedPrice ? ` · $${data.estimatedPrice}` : ''}`,
            data: { type: 'driver_new_order', deliveryId: data.deliveryId },
          });
        }
        navigation.navigate('TripRequest', { trip: tripData });
      }
    ...
    onRideTaken: () => {},
    onDeliveryTaken: () => {},
    // El usuario canceló: avisar al chofer (en cualquier pantalla), limpiar el
    // viaje activo y volver al inicio.
    onTripCancelled: (data) => {
      Alert.alert(
        data?.isDelivery ? 'Envío cancelado' : 'Viaje cancelado',
        data?.reason || 'El usuario canceló el pedido.',
        [{ text: 'Entendido' }]
      );
      setActiveTrip(null);
      setCurrentTrip(null);
  ...
  useEffect(() => {
    ...
    // Restaurar la conexión si el chofer quedó conectado: NO volver a pedir que
    // se conecte. Si no había presencia activa, limpiar disponibilidad colgada
    // (ej: app crasheó). El goOnline al socket se reemite en el efecto de abajo.
    driverPresence.load().then((presence) => {
      if (presence?.isActive) {
        setActiveServiceType(presence.serviceType || null);
        setIsOnline(true);
        setIsActive(true);
        driverPresence.startBackgroundUpdates();
        driverPresence.scheduleHourlyReminder();
      } else {
  ...
  // Reemitir goOnline cuando el socket está conectado y el chofer está activo.
  // Cubre: arranque restaurado desde background y reconexión al volver al foreground.
  useEffect(() => {
    if (socketConnected && isActive) {
      goOnline({
  ...
  const handleConnect = async () => {
    ...
    if (currentVehicle.documents_status?.all_approved !== true) {
      Alert.alert(
        ...
        [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Ver documentos', onPress: () => navigation.navigate('DriverMenuTab', { screen: 'Documents' }) },
        ]
      );
      return;
    }
  ...
  const handleStartReceiving = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Persistir presencia + ubicación en background + recordatorio cada hora,
        // para seguir disponible con la app minimizada hasta que se desconecte.
        driverPresence.start({
          serviceType: activeServiceType,
  ...
  const handleDisconnect = async () => {
    ...
    // Frenar presencia en background (ubicación + recordatorio) y olvidar el estado.
    driverPresence.stop();
    setIsOnline(false);
  };
    ...
    switch (driverStatus) {
      case 'pending_documents':
        return {
          ...
          subtitle: 'Recibimos tus datos y documentos. Nuestro equipo los está verificando antes de habilitarte.',
          icon: 'document-text-outline',
          color: COLORS.warning,
          action: null,
          steps: [
            { icon: 'checkmark-circle', text: 'Recibimos tu registro, tus documentos y tu vehículo.' },
            { icon: 'search', text: 'Estamos verificando que todo esté en orden (suele tardar entre 24 y 48 hs hábiles).' },
            { icon: 'notifications', text: 'Te avisamos por notificación apenas tu cuenta quede aprobada.' },
            { icon: 'car-sport', text: 'Una vez aprobada, vas a poder conectarte y empezar a recibir viajes y envíos.' },
          ],
          note: 'No necesitás hacer nada más por ahora. Si ya pasaron más de 48 hs hábiles, tocá "Verificar estado".',
        };
      case 'pending_review':
        return {
          title: 'Tu cuenta está en revisión',
          subtitle: 'Casi listo. Estamos haciendo la última revisión de tu cuenta antes de habilitarte.',
          icon: 'time-outline',
          color: COLORS.info,
          action: null,
          steps: [
            { icon: 'checkmark-circle', text: 'Tus documentos fueron recibidos correctamente.' },
            { icon: 'search', text: 'Un responsable de VNR está revisando tu perfil.' },
            { icon: 'notifications', text: 'Te notificamos en cuanto quede aprobado.' },
          ],
          note: 'No hace falta que hagas nada. Te avisamos apenas esté todo listo.',
        };
      case 'suspended':
        return {
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        <View style={styles.pendingContainer}>
          ...
          {statusInfo.steps && (
            <View style={{ alignSelf: 'stretch', marginHorizontal: 8, marginTop: 24, backgroundColor: COLORS.backgroundTertiary, borderRadius: 16, padding: 16 }}>
              <Text style={{ fontSize: 15, fontWeight: '700', color: COLORS.text, marginBottom: 12 }}>¿Qué sigue?</Text>
              {statusInfo.steps.map((s, i) => (
                <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 10 }}>
                  <Ionicons name={s.icon} size={18} color={statusInfo.color} style={{ marginTop: 1, marginRight: 10 }} />
                  <Text style={{ flex: 1, fontSize: 13.5, color: COLORS.textSecondary, lineHeight: 19 }}>{s.text}</Text>
                </View>
              ))}
              {statusInfo.note && (
                <Text style={{ fontSize: 12.5, color: COLORS.textMuted, lineHeight: 18, marginTop: 4, fontStyle: 'italic' }}>{statusInfo.note}</Text>
              )}
            </View>
          )}
  ...
  if (!isOnline) {
    return (
      <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        <View style={{ alignSelf: 'center', marginTop: 8, marginBottom: 4, backgroundColor: '#16A34A', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 999 }}>
          <Text style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 11, letterSpacing: 1.5 }}>VNR · CHOFER</Text>
        </View>
