...
const RateUserScreen = ({ navigation, route }) => {
  ...
  const { rideId, deliveryId, isDelivery, user, ratedId } = route.params || {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = isDelivery
        ? await ratingService.rateDelivery({ deliveryId, ...ratingPayload })
        : await ratingService.rateRide({ rideId, ...ratingPayload });
  ...
  const handleClose = () => {
    // Tras calificar (u omitir), volver a la home del chofer.
          ...
          {[1, 2, 3, 4, 5].map((rating) => (
            ...
            >
              <Ionicons
                ...
                size={44}
                color={rating <= stars ? '#FFD700' : 'rgba(255,255,255,0.55)'}
              />
            </TouchableOpacity>
          ))}
        </View>
