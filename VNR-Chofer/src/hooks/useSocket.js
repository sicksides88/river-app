...
export const useDriverSocket = (callbacks = {}) => {
  ...
  useEffect(() => {
    ...
    // Cancelación por parte del usuario (envío o viaje). Se maneja acá, en el
    // hook global, para que el chofer se entere esté en la pantalla que esté.
    const unsubDeliveryCancelled = socket.on('delivery:user_cancelled', (data) => {
      callbacksRef.current.onTripCancelled?.({ ...data, isDelivery: true });
    });
    const unsubRideCancelled = socket.on('ride:cancelled', (data) => {
      callbacksRef.current.onTripCancelled?.({ ...data, isDelivery: false });
    });
