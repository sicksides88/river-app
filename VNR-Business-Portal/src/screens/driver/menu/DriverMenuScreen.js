...
const DriverMenuScreen = ({ navigation }) => {
  ...
  const isDualRole = user?.selected_services?.some(s => DRIVER_SERVICES.includes(s)) &&
    user?.selected_services?.length >= 0; // Dual role si tiene servicios driver
  ...
  const renderMenuItem = (item) => (
    ...
    >
      <Ionicons
        name={item.icon}
        size={22}
        color={item.isDestructive ? (COLORS.error || '#DC2626') : COLORS.text}
      />
      <Text style={[
        styles.menuItemText,
