...
const TripActiveScreen = ({ navigation, route }) => {
  ...
  // Escuchar eventos de cancelación del usuario
  useEffect(() => {
    if (!isConnected || !socket || !tripId) return;

    // Cuando el usuario cancela el envío
    const unsubUserCancelled = socket.on('delivery:user_cancelled', (data) => {
      if (data.deliveryId === tripId) {
        Alert.alert(
          'Envío cancelado',
          data.reason || 'El usuario ha cancelado el envío.',
          [{
            text: 'OK',
    ...
    // También escuchar delivery:status_changed como backup
    const unsubStatusChanged = socket.on('delivery:status_changed', (data) => {
      if (data.deliveryId === tripId && data.status === 'cancelled' && data.cancelledBy === 'user') {
        Alert.alert(
          'Envío cancelado',
          data.reason || 'El usuario ha cancelado el envío.',
          [{
    ...
    // Para viajes (rides)
    const unsubRideCancelled = socket.on('ride:cancelled', (data) => {
      if (data.rideId === tripId && data.cancelledBy === 'user') {
        Alert.alert(
          'Viaje cancelado',
          data.reason || 'El usuario ha cancelado el viaje.',
          [{
            text: 'OK',
            onPress: () => navigation.navigate('DriverHome'),
          }]
  ...
  }, [isConnected, socket, tripId, navigation]);
