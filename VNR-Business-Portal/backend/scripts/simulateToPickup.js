/**
 * Script para simular el movimiento de un conductor hacia el punto de RETIRO
 * Uso: node scripts/simulateToPickup.js <deliveryId> [intervalMs]
 *
 * Ejemplo: node scripts/simulateToPickup.js abc123 2000
...
/**
 * Simular movimiento del conductor hacia el punto de retiro
 */
async function simulateToPickup(deliveryId, intervalMs = 2000) {
  log(colors.cyan, '🚗', `Simulación: Conductor hacia PUNTO DE RETIRO`);
  ...
  // Verificar estado válido para ir al pickup
  const validStates = ['pending', 'confirmed', 'accepted'];
  ...
  log(colors.green, '✅', `Delivery: ${delivery.pickup_address}`);
  ...
  // Obtener ubicación actual del conductor o simular una cercana
  let origin;
  ...
  if (driverAvail?.current_latitude) {
    ...
    log(colors.blue, '📍', `Ubicación actual del conductor: ${origin.lat}, ${origin.lng}`);
  } else {
    // Simular ubicación a ~1km del pickup
    origin = {
      lat: delivery.pickup_lat - 0.01,
      lng: delivery.pickup_lng - 0.01
    };
    log(colors.yellow, '📍', `Sin ubicación previa, simulando desde: ${origin.lat}, ${origin.lng}`);
  }

  const destination = { lat: delivery.pickup_lat, lng: delivery.pickup_lng };
  ...
  const moveDriver = async () => {
    if (currentIndex >= smoothPoints.length) {
      log(colors.green, '🎉', 'Conductor llegó al PUNTO DE RETIRO');

      // Actualizar estado a arrived_pickup
      await supabase
        .from('deliveries')
...
if (!deliveryId) {
  console.log(`
${colors.cyan}🚗 Simulador: Conductor hacia PUNTO DE RETIRO${colors.reset}

${colors.yellow}Uso:${colors.reset}
  node scripts/simulateToPickup.js <deliveryId> [intervalMs]

${colors.yellow}Ejemplo:${colors.reset}
  node scripts/simulateToPickup.js abc-123-def 1500

${colors.yellow}Estados válidos:${colors.reset}
  pending, confirmed, accepted

${colors.yellow}Al finalizar:${colors.reset}
  - Actualiza estado a: arrived_pickup
  `);
  process.exit(1);
}

simulateToPickup(deliveryId, intervalMs).catch((err) => {
