...
import { servicesService, toCanonicalService } from '../../services';
...
const HomeScreen = ({ navigation }) => {
  ...
  // Servicios habilitados por la plataforma (CRM). null = aún no cargó → mostrar todos.
  const [enabledServices, setEnabledServices] = useState(null);

  // Cargar servicios habilitados desde el CRM al montar
  useEffect(() => {
  ...
  // Una opción se muestra si todavía no cargaron los servicios habilitados (null = todos)
  // o si su service_type canónico está dentro de los habilitados por la plataforma.
  const isServiceEnabled = (opt) =>
    !enabledServices || enabledServices.includes(toCanonicalService(opt.id));

  const visibleServicesTabs = servicesTabs.filter(isServiceEnabled);
  const visibleServices = services.filter(isServiceEnabled);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        {/* Identidad de app (VNR Solicitante) */}
        <View style={{ alignSelf: 'flex-start', marginHorizontal: 16, marginTop: 8, marginBottom: 2, backgroundColor: '#2563EB', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 999 }}>
          <Text style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 11, letterSpacing: 1.5 }}>VNR · USUARIO</Text>
        </View>
