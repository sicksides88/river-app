...
import { authService, pushNotificationService, driverService, membershipService } from '../services';
...
export const AuthProvider = ({ children }) => {
  ...
  const checkAuth = async () => {
    try {
      ...
      await authService.restoreSession().catch(() => {});
      ...
      try {
        ...
        await membershipService.syncOnboardingFromUser(response.user);
        ...
        if (cachedUser && (authService.isRetriableAuthError(err) || __DEV__)) {
          if (__DEV__) {
            console.warn('[Auth] Sesión en caché — reload sin perder login (dev)');
          }
          setUser(cachedUser);
          setIsAuthenticated(true);
          return;
  ...
  const register = async (userData) => {
    try {
      ...
      if (err.response?.data?.message) {
        ...
        errorMessage =
          'No se pudo conectar al servidor. Verificá que el backend esté corriendo en tu PC (npm run dev).';
      }
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
